    ...
    if (!is.na(manta_normal_file))
      for (s in 1:length(manta_normal_file)) {
        ...
        if (length(vcf) > 0) {
          ...
          cat("Filter out variants seen 2+ (?) times in reference data\n")
